...
export function UnderstandingSidebar({ understanding }: UnderstandingSidebarProps) {
  ...
  const completenessPercent = Math.round(understanding.completeness_score * 100);
  const completenessColor = useMemo(() => {
    if (completenessPercent >= 70) return 'text-green-600 bg-green-100';
    if (completenessPercent >= 40) return 'text-amber-600 bg-amber-100';
    ...
    <div className="p-3 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium text-gray-900 capitalize">
              ...
              {understanding.classification.subcategory && (
                <span className="text-gray-500 font-normal"> ({understanding.classification.subcategory})</span>
              )}
            </span>
          </div>
        )}
      ...
      <div className="flex gap-4">
              ...
              {dimensions.slice(0, 6).map(([key, dim]) => (
                <div key={key} className="flex justify-between truncate">
                  ...
                  <span className="font-mono ml-1">
                    {dim.value}{dim.unit}
                    {dim.source === 'inferred' && <span className="text-amber-500">*</span>}
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {missing.length > 0 && (
            <div>
              <h4 className="font-medium text-amber-700 flex items-center gap-1 mb-1">
