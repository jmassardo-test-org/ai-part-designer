...
class TestClearanceDefaults:
    ...
    def test_all_processes_have_all_tolerances(self) -> None:
        ...
        for proc in PrintProcess:
            for tol in ToleranceClass:
                assert tol in CLEARANCE_DEFAULTS[proc], f"Missing {tol} for {proc}"
    ...
    def test_tight_less_than_standard_less_than_loose(self) -> None:
        ...
        for proc in PrintProcess:
            ...
            assert tight < standard < loose, (
                f"Clearance ordering violated for {proc}: {tight} < {standard} < {loose}"
            )
...
class TestMinPrintablePitch:
    ...
    def test_sla_allows_smaller_than_fdm(self) -> None:
        ...
        assert MIN_PRINTABLE_PITCH_MM[PrintProcess.SLA] < MIN_PRINTABLE_PITCH_MM[PrintProcess.FDM]
...
class TestGetPrintRecommendation:
    ...
    def test_excellent_has_higher_strength_than_marginal(
        ...
        assert rec_large.estimated_strength_pct >= rec_small.estimated_strength_pct
...
class TestOptimizeThreadForPrint:
    ...
    def test_loose_tolerance_larger_adjustment(
        ...
        loose_cfg = PrintThreadConfig(
            spec=m8_spec,
            tolerance_class=ToleranceClass.LOOSE,
        )
        ...
        assert loose_result.adjusted_spec.major_diameter < tight_result.adjusted_spec.major_diameter
    ...
    def test_tight_tolerance_smaller_adjustment(
        ...
        tight_diff = abs(m8_spec.major_diameter - tight_result.adjusted_spec.major_diameter)
        std_diff = abs(m8_spec.major_diameter - std_result.adjusted_spec.major_diameter)
    ...
    def test_includes_recommendation(
        ...
        assert isinstance(
            result.recommendation.feasibility,
            PrintFeasibility,
        )
    ...
    def test_sla_process_applied(
        ...
        result = optimize_thread_for_print(config)
        clearance = CLEARANCE_DEFAULTS[PrintProcess.SLA][ToleranceClass.STANDARD]
        assert result.adjusted_spec.major_diameter == pytest.approx(
            m8_spec.major_diameter - clearance,
        )
