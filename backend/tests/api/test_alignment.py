...
class TestAlignment:
    ...
    async def test_align_components_unauthenticated(self, client: AsyncClient):
        """Should return 401 without authentication."""
        response = await client.post("/api/v1/cad/align", json={})
        assert response.status_code == 401

    async def test_align_components(self, client: AsyncClient, auth_headers: dict):
...
class TestSnapToGrid:
    ...
    async def test_snap_to_grid(self, client: AsyncClient, auth_headers: dict):
