...
class TestFileUpload:
    ...
    async def test_upload_step_file_success(
        ...
        with patch("app.api.v1.files.get_settings") as mock_settings:
            ...
            response = await client.post(
                ...
                files={"file": ("test_part.step", io.BytesIO(file_content), "application/step")},
            )
    ...
    async def test_upload_stl_file_success(
        ...
        with patch("app.api.v1.files.get_settings") as mock_settings:
            ...
            response = await client.post(
                "/api/v1/files/upload",
                headers=auth_headers,
                files={"file": ("model.stl", io.BytesIO(file_content), "application/sla")},
            )
    ...
    async def test_upload_file_too_large(
        ...
        response = await client.post(
            "/api/v1/files/upload",
            headers=auth_headers,
            files={"file": ("large.step", io.BytesIO(large_content), "application/step")},
        )
    ...
    async def test_upload_invalid_file_type(
        ...
        response = await client.post(
            "/api/v1/files/upload",
            headers=auth_headers,
            files={"file": ("malware.exe", io.BytesIO(b"bad content"), "application/x-executable")},
        )
