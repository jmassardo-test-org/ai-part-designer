...
class TestCreateAnnotation:
    ...
    async def test_create_annotation_invalid_design(
        self, client: AsyncClient, auth_headers: dict
    ):
...
class TestGetAnnotation:
    ...
    async def test_get_annotation_unauthenticated(
        self, client: AsyncClient
    ):
        """Should return 401 without authentication."""
        fake_id = uuid4()
...
class TestAnnotationResolution:
    ...
    async def test_resolve_annotation_not_found(
        ...
        fake_id = uuid4()
        response = await client.post(
            f"/api/v1/annotations/{fake_id}/resolve",
            headers=auth_headers
        )
        
        assert response.status_code == 404

    async def test_reopen_annotation_not_found(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Should return 404 for non-existent annotation."""
        fake_id = uuid4()
        response = await client.post(
            f"/api/v1/annotations/{fake_id}/reopen",
            headers=auth_headers
        )
