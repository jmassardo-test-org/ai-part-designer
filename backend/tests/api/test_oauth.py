...
class TestOAuthLoginInitiation:
    ...
    async def test_google_oauth_login_returns_authorization_url(self, client: AsyncClient):
    ...
    async def test_github_oauth_login_returns_authorization_url(self, client: AsyncClient):
    ...
    async def test_google_oauth_not_configured_returns_503(self, client: AsyncClient):
    ...
    async def test_github_oauth_not_configured_returns_503(self, client: AsyncClient):
    ...
    async def test_oauth_misconfigured_returns_helpful_error(self, client: AsyncClient):
...
class TestOAuthCallback:
    ...
    async def test_github_callback_without_code(self, client: AsyncClient):
        """Should handle missing authorization code."""
        response = await client.get("/api/v1/auth/oauth/github/callback")

        # Should be error without code
        assert response.status_code in [400, 401, 422, 404, 302, 307]

    async def test_google_callback_with_invalid_code(self, client: AsyncClient):
...
class TestOAuthUserCreation:
    ...
    async def test_oauth_creates_new_user(self, client: AsyncClient, db_session):
    ...
    async def test_oauth_links_existing_user(self, client: AsyncClient, db_session):
        """Should link OAuth to existing user with same email."""

    async def test_oauth_duplicate_connection_handled(self, client: AsyncClient, db_session):
...
class TestOAuthConnectionManagement:
    ...
    async def test_list_oauth_connections_authenticated(self, client: AsyncClient, auth_headers):
    ...
    async def test_list_oauth_connections_unauthenticated(self, client: AsyncClient):
        """Should reject unauthenticated requests."""
        response = await client.get("/api/v1/auth/oauth/connections")

        assert response.status_code in [401, 403]

    async def test_disconnect_oauth_provider(self, client: AsyncClient, auth_headers):
        """Should allow disconnecting an OAuth provider."""
        response = await client.delete(
            "/api/v1/auth/oauth/connections/google", headers=auth_headers
        )
