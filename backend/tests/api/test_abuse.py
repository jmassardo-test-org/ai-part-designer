...
class TestAbuseDashboard:
    ...
    async def test_dashboard_non_admin(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Should return 403 for non-admin users."""
        response = await client.get(
            "/api/v1/admin/abuse/dashboard",
            headers=auth_headers
        )
        assert response.status_code == 403

    async def test_dashboard_admin(
...
class TestAbuseReports:
    ...
    async def test_list_reports_non_admin(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Should return 403 for non-admin users."""
        response = await client.get(
...
class TestBanManagement:
    ...
    async def test_list_bans_unauthenticated(
        self, client: AsyncClient
    ):
        """Should return 401 without authentication."""
    ...
    async def test_list_bans_non_admin(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Should return 403 for non-admin users."""
        response = await client.get(
            "/api/v1/admin/abuse/bans",
            headers=auth_headers
        )
        assert response.status_code == 403

    async def test_list_bans_admin(
        self, client: AsyncClient, admin_headers: dict
    ):
...
class TestModerationQueue:
    ...
    async def test_queue_unauthenticated(
        self, client: AsyncClient
    ):
        """Should return 401 without authentication."""
    ...
    async def test_queue_non_admin(
        self, client: AsyncClient, auth_headers: dict
    ):
        """Should return 403 for non-admin users."""
        response = await client.get(
            "/api/v1/admin/abuse/moderation-queue",
            headers=auth_headers
        )
        assert response.status_code == 403

    async def test_queue_admin(
        self, client: AsyncClient, admin_headers: dict
    ):
