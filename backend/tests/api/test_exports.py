...
class TestExports:
    ...
    async def test_list_exports_success(self, client: AsyncClient, auth_headers: dict):
...
class TestExportRequest:
    ...
    async def test_request_export_success(self, client: AsyncClient, auth_headers: dict):
        """Should request export of user's data."""
        response = await client.post("/api/v1/exports/request", headers=auth_headers, json={})
    ...
    async def test_request_export_unauthenticated(self, client: AsyncClient):
        """Should reject unauthenticated export request."""
        response = await client.post("/api/v1/exports/request", json={})
