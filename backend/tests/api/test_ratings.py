...
from tests.factories import Counter, TemplateFactory
...
class TestTemplateRatings:
    ...
    async def test_get_template_ratings(self, client: AsyncClient, db_session: AsyncSession):
        """Anyone can view template ratings."""
        template = await TemplateFactory.create(db_session, is_active=True)

        response = await client.get(f"/api/v1/templates/{template.id}/ratings")
    ...
    async def test_get_my_rating(
        ...
        response = await client.get(
            f"/api/v1/templates/{template.id}/ratings/me", headers=auth_headers
        )
    ...
    async def test_delete_rating(
        ...
        response = await client.delete(
            f"/api/v1/templates/{template.id}/ratings", headers=auth_headers
        )
...
class TestTemplateFeedback:
    ...
    async def test_get_feedback_summary(
        ...
        response = await client.get(
            f"/api/v1/templates/{template.id}/feedback", headers=auth_headers
        )
