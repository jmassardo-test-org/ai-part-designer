...
class TestSaveDesign:
    ...
    async def test_save_design_creates_default_list(
        self, auth_client: AsyncClient, other_user_design: Design, db_session: AsyncSession, test_user
    ):
    ...
    async def test_save_already_saved_design_adds_to_new_lists(
        self, auth_client: AsyncClient, other_user_design: Design, user_lists: list[DesignList], db_session: AsyncSession, test_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_save_nonexistent_design_returns_404(
...
class TestCheckSaveStatus:
    ...
    @pytest.mark.asyncio
    async def test_check_unsaved_design(
        self, auth_client: AsyncClient, other_user_design: Design
    ):
        """Test checking status of unsaved design."""
        response = await auth_client.get(
            f"/api/v2/saves/{other_user_design.id}/check"
        )
