...
class TestDesignDetail:
    ...
    @pytest.mark.asyncio
    async def test_get_nonexistent_design_returns_404(
...
class TestPublishDesign:
    ...
    @pytest.mark.asyncio
    async def test_publish_design_success(
    ...
    @pytest.mark.asyncio
    async def test_publish_nonexistent_design_returns_404(
        self, auth_client: AsyncClient
    ):
...
class TestUnpublishDesign:
    ...
    async def test_unpublish_design_success(
        ...
        response = await auth_client.post(
            f"/api/v2/marketplace/designs/{design.id}/unpublish"
        )
