...
class TestAuthSecurity:
    ...
    @pytest.mark.asyncio
    async def test_password_not_in_response(self, client: AsyncClient, auth_headers: dict):
    ...
    async def test_jwt_algorithm_enforcement(self, client: AsyncClient):
        ...
        response = await client.get(
            "/api/v1/auth/me", headers={"Authorization": f"Bearer {token_none_alg}"}
        )
...
class TestBruteForceProtection:
    ...
    @pytest.mark.asyncio
    async def test_login_returns_same_error_for_invalid_user(self, client: AsyncClient):
