...
class TestFileUploadSecurity:
    ...
    async def test_file_type_validation(
        ...
        files = {
            "file": ("malicious.step", BytesIO(malicious_content), "application/octet-stream")
        }
    ...
    async def test_path_traversal_prevention(
        ...
        for filename in path_traversal_names:
            files = {
                "file": (filename, BytesIO(b"test content"), "text/plain")
            }
    ...
    async def test_file_size_limit_enforced(
        ...
        files = {
            "file": ("large.step", BytesIO(large_content), "application/step")
        }
    ...
    @pytest.mark.asyncio
    async def test_double_extension_handling(
        self, client: AsyncClient, auth_headers: dict
    ):
        ...
        for filename in dangerous_names:
            files = {
                "file": (filename, BytesIO(b"test content"), "application/octet-stream")
            }
    ...
    async def test_null_byte_injection(
        ...
        files = {
            "file": (filename, BytesIO(b"test content"), "application/step")
        }
