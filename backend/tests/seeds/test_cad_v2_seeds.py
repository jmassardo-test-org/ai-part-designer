...
class TestComponentV2Seeding:
    ...
    async def test_seed_components_v2_creates_records(self, db_session: AsyncSession):
    ...
    async def test_seed_components_v2_sets_library_source(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(
            select(ReferenceComponent).where(ReferenceComponent.source_type == "library")
        )
    ...
    async def test_seed_components_v2_includes_dimensions(self, db_session: AsyncSession):
        """Seeded components have dimensions data."""
        from app.seeds.components_v2 import seed_components_v2

        await seed_components_v2(db_session)

        result = await db_session.execute(select(ReferenceComponent).limit(5))
        components = result.scalars().all()
...
class TestStarterSeeding:
    ...
    async def test_seed_starters_creates_vendor_user(self, db_session: AsyncSession):
        """Seeding creates the vendor user."""
        from app.seeds.starters import VENDOR_USER_ID, seed_starters

        await seed_starters(db_session)

        result = await db_session.execute(select(User).where(User.id == VENDOR_USER_ID))
        vendor = result.scalar_one_or_none()

        assert vendor is not None
        assert vendor.role == "system"

    async def test_seed_starters_creates_project(self, db_session: AsyncSession):
        """Seeding creates the starters project."""
        from app.seeds.starters import STARTERS_PROJECT_ID, seed_starters

        await seed_starters(db_session)

        result = await db_session.execute(select(Project).where(Project.id == STARTERS_PROJECT_ID))
        project = result.scalar_one_or_none()

        assert project is not None
        assert project.name == "Starter Designs"

    async def test_seed_starters_creates_designs(self, db_session: AsyncSession):
    ...
    async def test_seed_starters_is_idempotent(self, db_session: AsyncSession):
        """Running seed twice doesn't duplicate records."""
        from app.seeds.starters import STARTER_DESIGNS, seed_starters

        # First run
        _created1, _updated1 = await seed_starters(db_session)
    ...
    async def test_seeded_starters_are_marked_correctly(self, db_session: AsyncSession):
        """Seeded starters have correct flags."""
        from app.seeds.starters import seed_starters

        await seed_starters(db_session)

        result = await db_session.execute(select(Design).where(Design.source_type == "starter"))
    ...
    async def test_seeded_starters_have_categories_and_tags(self, db_session: AsyncSession):
...
class TestSeedingIntegration:
    ...
    async def test_full_seeding_workflow(self, db_session: AsyncSession):
        """Test complete seeding workflow."""
        from app.seeds.components_v2 import seed_components_v2
        from app.seeds.starters import seed_starters

        # Seed components
        comp_created, _comp_updated = await seed_components_v2(db_session)

        # Seed starters
        start_created, _start_updated = await seed_starters(db_session)
        ...
        starters = await db_session.execute(select(Design).where(Design.source_type == "starter"))
