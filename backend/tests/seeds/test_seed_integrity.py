...
class TestTemplateSeedIntegrity:
    ...
    def test_all_seeded_templates_have_generators(self):
        ...
        assert len(missing_generators) == 0, (
            f"Templates missing generators: {missing_generators}"
        )
        ...
        for template in TEMPLATE_SEEDS:
            ...
            for param_name, param_def in params.items():
                ...
                if param_type == "number":
                    assert "min" in param_def or "max" in param_def or True, (
                        f"Number param '{param_name}' should have constraints"
                    )
...
class TestUserSeedIntegrity:
    ...
    def test_user_seeds_have_required_fields(self):
        ...
        from app.seeds.users import PLATFORM_ADMIN, FREE_USERS
    ...
    def test_user_emails_are_unique(self):
        ...
        assert len(emails) == len(unique_emails), (
            f"Duplicate emails found in user seeds"
        )
    ...
    def test_admin_user_has_admin_role(self):
        ...
        assert PLATFORM_ADMIN.get("role") == "admin", (
            "Platform admin should have admin role"
        )
            ...
            if generator:
                try:
                    ...
                    assert result is not None, (
                        f"Generator for '{slug}' returned None"
                    )
...
class TestSeedCrossReferences:
    ...
    def test_template_categories_are_not_empty(self):
        ...
        for template in TEMPLATE_SEEDS:
            ...
            assert category, (
                f"Template '{template.get('name')}' has empty category"
            )
            assert isinstance(category, str), (
                f"Template '{template.get('name')}' category is not a string"
            )
            assert len(category) > 0, (
                f"Template '{template.get('name')}' has empty category string"
            )
