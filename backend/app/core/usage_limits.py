...
class ConcurrentOperation(Base):
    ...
    __table_args__ = (
        Index("ix_concurrent_user_type", "user_id", "operation_type"),
    )
...
class UsageLimitService:
    ...
    async def end_concurrent_operation(
        ...
        query = select(ConcurrentOperation).where(
            ConcurrentOperation.id == operation_id
        )
        result = await self.db.execute(query)
    ...
    async def _cleanup_expired_operations(self) -> int:
        ...
        stmt = delete(ConcurrentOperation).where(
            ConcurrentOperation.expires_at < datetime.utcnow()
        )
