    ...
    try:
        ...
        if not existing_connection or not user:
            ...
            oauth_connection = OAuthConnection(
                ...
                token_expires_at=(
                    datetime.now(tz=UTC) + timedelta(seconds=token["expires_in"])
            ...
            if token.get("expires_in"):
                existing_connection.token_expires_at = datetime.now(tz=UTC) + timedelta(
                    seconds=token["expires_in"]
                )
            existing_connection.profile_data = user_info
            existing_connection.last_used_at = datetime.now(tz=UTC)
...
async def link_oauth_callback(
    ...
    try:
        ...
        if existing_conn:
            ...
            existing_conn.last_used_at = datetime.now(tz=UTC)
        else:
