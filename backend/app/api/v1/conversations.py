...
from app.ai.codegen import generate_modification
...
def _understanding_to_intent(understanding: PartUnderstanding) -> PartIntent:
    ...
    for key, dim in understanding.dimensions.items():
    ...
    if understanding.classification:
        ...
        if understanding.classification.subcategory:
            primary_function = understanding.classification.subcategory
        else:
            primary_function = f"Standard {part_type}"
...
def _is_modification_request(message: str) -> bool:
    ...
    modification_keywords = [
        "add ", "add a ", "add an ",
        "remove ", "remove the ", "delete ", "delete the ",
        "make it ", "make the ", "change ", "change the ",
        "increase ", "decrease ", "resize ",
        "move ", "move the ", "rotate ",
        "add hole", "add holes", "drill ", "cut ", "cut a ",
        "fillet ", "chamfer ", "round ",
        "taller", "shorter", "wider", "narrower", "longer", "deeper",
        "bigger", "smaller", "thicker", "thinner",
        "modify", "update", "adjust",
        "put a ", "include ", "attach ",
        "another ", "additional ", "extra ",
    ]
    
    message_lower = message.lower().strip()
    return any(message_lower.startswith(kw) or f" {kw}" in message_lower 
    ...
    try:
        ...
        if understanding.state == ReasoningState.NEEDS_CLARIFICATION:
            ...
            try:
                ...
                is_modification = (
                    conversation.result_data is not None 
                    and _is_modification_request(request.content)
                )
                ...
                if is_modification:
                    ...
                    if not original_code:
                        ...
                        full_description = " ".join(understanding.user_messages) + " " + request.content
                        from app.ai.direct_generation import generate_directly
                    ...
                    class DirectResult:
                        def __init__(self):
                            ...
                            self.dimensions = {k: v.value for k, v in understanding.dimensions.items()}
                            self.confidence = 0.9
                            self.warnings = []
