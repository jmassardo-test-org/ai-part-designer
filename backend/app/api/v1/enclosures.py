...
from app.enclosure.prompts import ComponentData
...
class EnclosureJobStatus(BaseModel):
    ...
    status: str
    progress: int = 0
    result: Optional[EnclosureResult] = None
...
async def list_style_templates(
    ...
    return StyleTemplateListResponse(
        styles=[StyleTemplateInfo(**s) for s in styles]
    )
