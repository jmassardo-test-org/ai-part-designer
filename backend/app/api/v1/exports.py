...
async def request_data_export(
    ...
    recent_cutoff = datetime.now(tz=UTC) - timedelta(hours=24)
    for job in _export_jobs.values():
        if (
            job.get("user_id") == user_id_str
            and job.get("requested_at", datetime.min.replace(tzinfo=UTC)) > recent_cutoff
        ):
            raise HTTPException(
    ...
    return ExportRequestResponse(
        ...
        estimated_completion=datetime.now(tz=UTC) + timedelta(minutes=5),
