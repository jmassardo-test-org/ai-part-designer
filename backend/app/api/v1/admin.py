...
async def escalate_content(
    ...
    item.details["escalated_at"] = datetime.now(UTC).isoformat()
...
async def ban_user(
    ...
    if not request.is_permanent and request.duration_days:
        expires_at = datetime.now(UTC) + timedelta(days=request.duration_days)
...
async def suspend_user(
    ...
    user.extra_data["suspended_at"] = datetime.now(UTC).isoformat()
    user.extra_data["suspended_by"] = str(current_user.id)
    if request.duration_days:
        user.extra_data["suspension_expires"] = (
            datetime.now(UTC) + timedelta(days=request.duration_days)
        ).isoformat()
...
async def extend_subscription(
    ...
    if sub.current_period_end:
        ...
        sub.current_period_end = datetime.now(UTC) + timedelta(days=request.days)
...
async def update_starter(
    ...
    starter.updated_at = datetime.now(UTC)
