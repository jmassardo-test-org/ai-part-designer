...
class ComponentFileStorage:
    ...
    async def store_datasheet(
        ...
        if len(content) > MAX_DATASHEET_SIZE:
            raise ValueError(f"File too large. Maximum size: {MAX_DATASHEET_SIZE // (1024*1024)}MB")
    ...
    async def get_datasheet(self, component_id: UUID, file_id: UUID) -> Optional[Path]:
    ...
    async def get_cad_file(self, component_id: UUID, file_id: UUID) -> Optional[Path]:
    ...
    async def get_thumbnail(self, component_id: UUID, size: str = "medium") -> Optional[Path]:
