...
class ModerationResult:
    ...
    def highest_severity(self) -> FlagSeverity | None:
        ...
        max_idx = max(severity_order.index(f.severity) for f in self.flags)
...
FILENAME_PATTERNS: list[tuple[str, ContentCategory, FlagSeverity]] = [
    # Weapons
    (
        r"(?:^|[\W_])(gun|pistol|rifle|firearm|weapon)(?:[\W_]|$)",
        ContentCategory.WEAPON,
        FlagSeverity.HIGH,
    ),
    (
        r"(?:^|[\W_])(ar[-_]?15|ak[-_]?47|glock|m16)(?:[\W_]|$)",
        ContentCategory.WEAPON,
        FlagSeverity.CRITICAL,
    ),
    (
        r"(?:^|[\W_])(receiver|lower|upper|trigger|barrel)(?:[\W_]|$)",
        ContentCategory.WEAPON_COMPONENT,
        FlagSeverity.MEDIUM,
    ),
    (
        r"(?:^|[\W_])(suppressor|silencer)(?:[\W_]|$)",
        ContentCategory.WEAPON_COMPONENT,
        FlagSeverity.CRITICAL,
    ),
    (
        r"(?:^|[\W_])(magazine|mag[-_]?clip)(?:[\W_]|$)",
        ContentCategory.WEAPON_COMPONENT,
        FlagSeverity.MEDIUM,
    ),
    # Drug paraphernalia
    (
        r"(?:^|[\W_])(pipe|bong|grinder)(?:[\W_]|$)",
        ContentCategory.DRUG_PARAPHERNALIA,
        FlagSeverity.LOW,
    ),
    # Counterfeit
    (
        r"(?:^|[\W_])(replica|fake|counterfeit)(?:[\W_]|$)",
        ContentCategory.COUNTERFEIT,
        FlagSeverity.MEDIUM,
    ),
    # Restricted
    (
        r"(?:^|[\W_])(itar|munition|export[-_]?control)(?:[\W_]|$)",
        ContentCategory.RESTRICTED_EXPORT,
        FlagSeverity.HIGH,
    ),
]
...
GEOMETRY_SIGNATURES: list[dict[str, Any]] = [
    {
        ...
        "check": lambda info: (
            info.get("aspect_ratio", 0) > 3.0
            and info.get("has_internal_cavity", False)
            and info.get("cavity_ratio", 0) > 0.3
        ),
    ...
    },
    {
        ...
        "check": lambda info: (
            0.5 < info.get("aspect_ratio", 0) < 2.0
            and info.get("internal_feature_count", 0) > 5
            and info.get("has_pin_holes", False)
        ),
        "category": ContentCategory.WEAPON_COMPONENT,
        "severity": FlagSeverity.MEDIUM,
        "confidence": 0.35,
    },
    {
        ...
        "check": lambda info: (
            info.get("aspect_ratio", 0) > 2.0
            and info.get("has_internal_cavity", False)
            and info.get("wall_thickness", 0) < 3.0
        ),
        "category": ContentCategory.WEAPON_COMPONENT,
...
class ContentModerator:
    ...
    async def analyze_file(
        ...
        # Auto-decision logic
        return self._apply_auto_decision(mod_result)
    ...
    def _extract_geometry_features(self, shape: Part) -> dict:
        ...
        try:
            ...
            features["fill_ratio"] = (
                features["volume"] / expected_solid_volume if expected_solid_volume > 0 else 1.0
            )
