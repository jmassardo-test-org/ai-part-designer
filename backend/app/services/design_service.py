...
class DesignService:
    ...
    async def copy_design(
        ...
        logger.info(
            f"Copying design {design_id} to project {target_project.id} "
            f"for user {user.id}"
        )
    ...
    async def restore_version(
        ...
        restored_version = DesignVersion(
            ...
            geometry_info=target_version.geometry_info.copy() if target_version.geometry_info else {},
