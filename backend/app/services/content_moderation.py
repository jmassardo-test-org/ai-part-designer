...
class ContentModerationService:
    ...
    def _compile_patterns(self):
        ...
        self.zero_tolerance_regex = [
            re.compile(p, re.IGNORECASE) for p in ZERO_TOLERANCE_PATTERNS
        ]
        ...
        self.allowlist_regex = [
            re.compile(p, re.IGNORECASE) for p in ALLOWLIST_PATTERNS
        ]
        self.off_topic_regex = [
            re.compile(p, re.IGNORECASE) for p in OFF_TOPIC_PATTERNS
        ]
        self.design_intent_regex = [
            re.compile(p, re.IGNORECASE) for p in DESIGN_INTENT_PATTERNS
        ]
        self.api_abuse_regex = [
            re.compile(p, re.IGNORECASE) for p in API_ABUSE_PATTERNS
        ]
        ...
        if use_ai and (pattern_flags or result.flags or not result.is_allowlisted):
            ...
            if ai_result:
                if ai_result.get("decision") == "reject":
                    result.flags.append(ModerationFlag(
                        category=self._map_ai_category(ai_result.get("category", "other_prohibited")),
                        severity="high",
    ...
    def _check_api_abuse(self, prompt: str) -> list[ModerationFlag]:
        ...
        for pattern in self.api_abuse_regex:
            ...
            if match:
                ...
                injection_keywords = ["ignore", "pretend", "roleplay", "jailbreak", "bypass", "reveal", "system"]
                is_injection = any(kw in matched_text.lower() for kw in injection_keywords)
                
                flags.append(ModerationFlag(
                    category=ProhibitedCategory.PROMPT_INJECTION if is_injection else ProhibitedCategory.API_ABUSE,
                    severity="critical",
    ...
    def _check_design_intent(self, prompt: str) -> bool:
        ...
        for pattern in self.design_intent_regex:
            if pattern.search(prompt):
                return True
        return False
    
    def _check_allowlist(self, prompt: str) -> Optional[str]:
    ...
    def _categorize_match(self, matched_text: str) -> ProhibitedCategory:
        ...
        firearm_keywords = [
            "receiver", "ar-15", "ar15", "ak-47", "ak47", "glock", "m16",
            "trigger", "barrel", "suppressor", "silencer", "magazine",
            "bolt", "firing pin", "ghost gun", "80%", "fgc-9",
        ]
    ...
    def get_rejection_message(self, result: ModerationResult) -> str:
        ...
        if ProhibitedCategory.PROMPT_INJECTION in categories or ProhibitedCategory.API_ABUSE in categories:
            return (
        ...
        elif result.decision == ModerationDecision.REVIEW_REQUIRED:
            return (
