...
if TYPE_CHECKING:
    from app.models.rating import TemplateComment, TemplateFeedback, TemplateRating
        ...
        for name, definition in self.parameters.items():
            ...
            if param_type == "number":
                ...
                if "min" in definition and value < definition["min"]:
                    errors.append(f"Parameter '{name}' must be >= {definition['min']}")
                if "max" in definition and value > definition["max"]:
                    errors.append(f"Parameter '{name}' must be <= {definition['max']}")

            elif param_type == "enum":
                options = definition.get("options", [])
                if value not in options:
                    errors.append(f"Parameter '{name}' must be one of: {', '.join(options)}")
