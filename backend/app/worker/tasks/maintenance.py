...
def purge_expired_trash() -> dict[str, Any]:
    ...
    from app.models import Design, Project, File
    import asyncio
    ...
    async def run():
        ...
        total_deleted = (
            deleted_summary["designs"] +
            deleted_summary["projects"] +
            deleted_summary["files"]
        )
...
def send_trash_deletion_warnings() -> dict[str, Any]:
    ...
    from app.models import User, Design, Project, File
    import asyncio
            ...
            for user in users:
                ...
                for warning_day in WARNING_DAYS:
                    ...
                    designs_result = await session.execute(
                        ...
                        .where(Design.deleted_at.isnot(None))
                        .where(and_(
                            Design.deleted_at >= cutoff_start,
                            Design.deleted_at < cutoff_end
                        ))
                    )
                    ...
                    projects_result = await session.execute(
                        ...
                        .where(Project.deleted_at.isnot(None))
                        .where(and_(
                            Project.deleted_at >= cutoff_start,
                            Project.deleted_at < cutoff_end
                        ))
                    )
    ...
    async def run():
        async with async_session_maker() as session:
            ...
            result = await session.execute(
                ...
                .where(DesignVersion.thumbnail_url.is_(None))
                .limit(100)
            )
...
def check_storage_health() -> dict:
    ...
    async def run():
        ...
        for bucket in StorageBucket:
            try:
                files = await storage_client.list_files(bucket, max_keys=1)
    ...
    async def run():
        async with async_session_maker() as session:
            ...
            if report.is_healthy:
                logger.info(
                    f"Integrity check passed - {len(report.checks_run)} checks, "
                    f"no critical issues"
                )
            else:
                logger.warning(
                    f"Integrity check found issues: {report.issue_counts}"
                )
...
def verify_backups() -> dict[str, Any]:
    ...
    async def run():
        ...
        recent_backups = [
            record for record in backup_service._backup_index.values()
            ...
            try:
                ...
                if result.is_valid:
                    ...
                    for issue in result.issues:
                        ...
                        logger.error(
                            f"Backup {record.id} verification failed: {issue}"
                        )
