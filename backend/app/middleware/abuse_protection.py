...
from app.core.redis_rate_limit import get_rate_limiter, get_rate_limit_key
...
class AbuseProtectionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        rate_limit_response = await self._check_rate_limit(
            request, client_ip, path, is_suspicious
        )
...
class GenerationGuardMiddleware:
    ...
    async def check_generation_allowed(
        ...
        if mod_result.is_rejected:
            ...
            async with self.db_session_factory() as db:
                ...
                violation = ViolationEvent(
                    ...
                    severity="critical",
                    description=f"Prohibited content detected in generation prompt",
                    evidence={
                        "prompt": prompt[:500],
                        "flags": [
        ...
        async with self.db_session_factory() as db:
            ...
            if not allowed:
                return False, f"Generation limit reached: {details.get('reason', 'limit exceeded')}", details
            ...
            if not concurrent_allowed:
                return False, f"Too many concurrent generations. Please wait for current jobs to complete.", {
                    "current": current,
                }
