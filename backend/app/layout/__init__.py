...
class Placement:
    ...
    def intersects(self, other: "Placement", margin: float = 0) -> bool:
        ...
        if self.z + self.component.height + margin <= other.z:
            return False
        if other.z + other.component.height + margin <= self.z:
            return False
...
class LayoutResult:
    ...
    def utilization(self) -> float:
        ...
        component_area = sum(
            p.component.footprint_area for p in self.placements
        )
...
class AutoLayoutEngine:
    ...
    def layout(
        ...
        elif algorithm == LayoutAlgorithm.THERMAL:
            return self._thermal_layout(components, max_width, max_depth)
        elif algorithm == LayoutAlgorithm.CONNECTOR:
    ...
    def _thermal_layout(
        ...
        if len(high_power) <= 1:
            ...
            return self._packed_layout(
                components, max_width, max_depth, prioritize_connectors=True
            )
        ...
        if remaining:
            ...
            for comp in remaining:
                ...
                test_placement = Placement(
                    component=comp, x=current_x, y=current_y, z=0
                )
                
                collision = any(
                    test_placement.intersects(p, self.clearance)
                    for p in placements
                )
    ...
    def _connector_layout(
        ...
        if max_width is None:
            ...
            max_width = max(
                max_comp_width * 3,
                sum(c.rotated_width for c in has_front_connectors) + self.clearance * len(has_front_connectors),
            ) + self.wall_margin * 2
        ...
        current_x = self.wall_margin
        back_y = self.wall_margin + front_row_depth + self.clearance * 2 + max(
    ...
    def validate_layout(
        ...
        if max_width and result.enclosure_width > max_width:
            errors.append(
                f"Layout exceeds max width: {result.enclosure_width:.1f} > {max_width}"
            )
        if max_depth and result.enclosure_depth > max_depth:
            errors.append(
                f"Layout exceeds max depth: {result.enclosure_depth:.1f} > {max_depth}"
            )
        ...
        for i, p1 in enumerate(placements):
            for p2 in placements[i + 1:]:
                if p1.intersects(p2, margin=0):
                    errors.append(
                        f"Collision between {p1.component.name} and {p2.component.name}"
                    )
        ...
        for p in placements:
            if p.x < 0 or p.y < 0 or p.z < 0:
                errors.append(
                    f"Component {p.component.name} has negative position"
                )
