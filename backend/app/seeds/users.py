...
async def create_user(
    ...
    user = User(
        ...
        last_login_at=datetime.now(tz=UTC) - timedelta(days=1),
        extra_data={
            "org_name": org_name,
            "seeded": True,
    ...
    subscription = Subscription(
        ...
        tier=tier,
        status="active",
        current_period_start=datetime.now(tz=UTC) - timedelta(days=15),
        current_period_end=datetime.now(tz=UTC) + timedelta(days=15),
    )
