...
class CADParameters(BaseModel):
    ...
    features: list[Feature] = Field(
        ...
        description="Additional features like holes, fillets"
    )
    units: str = Field(
        default="mm",
        description="Original units from user input"
    )
    ...
    assembly_type: AssemblyType = Field(
        ...
        description="Type of multi-part assembly"
    )
    assembly_config: dict[str, Any] = Field(
        default_factory=dict,
        description="Assembly-specific configuration"
    )
...
class DescriptionParser:
    ...
    def _normalize_box_dimensions(self, dims: dict[str, float]) -> dict[str, float]:
        ...
        for dim, val in zip(missing, unused_values):
            result[dim] = val
