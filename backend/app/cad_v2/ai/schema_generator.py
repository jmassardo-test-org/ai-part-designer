...
from app.cad_v2.ai.intent import IntentParser, ParsedIntent, IntentType
from app.cad_v2.ai.prompts import ENCLOSURE_GENERATION, CAD_V2_SYSTEM_PROMPT
...
class SchemaGenerator:
        ...
        *,
        ...
        if parse_intent:
            ...
            if intent.intent_type == IntentType.UNCLEAR:
                return GenerationResult(
                    spec=None,
                    success=False,
                    clarification_needed=intent.clarification_needed or 
                        "Could you provide more details about what you'd like to create?",
                )
        ...
        try:
            ...
            return GenerationResult(
                spec=None,
                success=False,
                validation_errors=[f"AI error: {str(e)}"],
            )
    ...
    def _check_warnings(
        ...
        if max_dim > 300:
            warnings.append(
                f"Maximum dimension {max_dim}mm may exceed typical "
                f"3D printer bed size (300mm)"
            )
