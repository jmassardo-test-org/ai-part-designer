...
from build123d import (
    Part, Compound, Solid, Location, Vector, Axis,
)
...
class TransformationResult:
    ...
    rotation: Optional[Tuple[float, float, float]] = None  # Euler angles in degrees
...
class AlignmentService:
    ...
    def rotate_shape(
        ...
        result = self.translate_shape(rotated, cx, cy, cz)
    ...
    def side_by_side(
        ...
        return self.stack_shapes(
            ...
            axis=axis, 
            gap=gap, 
            center_other_axes=not align_bottoms
        )
    ...
    def align_shapes(
        ...
        elif mode == AlignmentMode.STACK_Z:
            return self.stack_shapes([reference, target], AlignmentAxis.Z, gap=options.get("gap", 0))
        
        elif mode == AlignmentMode.STACK_X:
            return self.stack_shapes([reference, target], AlignmentAxis.X, gap=options.get("gap", 0))
        
        elif mode == AlignmentMode.STACK_Y:
            return self.stack_shapes([reference, target], AlignmentAxis.Y, gap=options.get("gap", 0))
        
        elif mode == AlignmentMode.FACE:
