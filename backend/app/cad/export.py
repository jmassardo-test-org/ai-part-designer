    ...
    try:
        ...
        try:
            ...
            if status != 1:  # IFSelect_RetDone = 1
                raise ExportError(
                    "STEP write failed", details={"format": "STEP", "status_code": status}
                )
        ...
        raise ExportError(f"STEP export failed: {e}", details={"format": "STEP"})
...
def convert_cad_format(
    ...
    *,
    ...
    if not source_path.exists():
        raise ValidationError(
            f"Source file not found: {source_path}", details={"path": str(source_path)}
        )
    ...
    try:
        if source_ext in (".step", ".stp"):
            ...
            raise ValidationError(
                ...
                details={
                    "source_format": "stl",
                    "message": "STL is mesh-only and cannot be converted to solid formats",
                },
            )
        else:
            raise ValidationError(
        ...
        raise ExportError(f"Failed to load source file: {e}", details={"source": str(source_path)})
