...
from build123d import (
    Box, BuildSketch, Cylinder, Location, Part,
    Axis, Align, Polygon, RegularPolygon, Circle,
    fillet, extrude, Pos, Polyline, make_face, Vector,
)
...
class SnapFitGenerator:
    ...
    def _generate_cantilever(self) -> Part:
        ...
        result = Box(
            cfg.width, cfg.thickness * 2, cfg.thickness * 3,
        ...
        beam = Box(
            cfg.length, cfg.width, beam_height,
        ...
        hook = Box(
            cfg.hook_undercut + 2, cfg.width, cfg.hook_height,
        ...
        undercut = Box(
            cfg.hook_undercut, cfg.width * 0.8, cfg.hook_height * 0.7,
    ...
    def generate_receptacle(self, clearance: float = 0.3) -> Part:
        ...
        # Outer block
        result = Box(
            pocket_width + 4, pocket_length + 4, pocket_depth + 2,
            align=(Align.CENTER, Align.CENTER, Align.CENTER)
        )
...
class DINRailGenerator:
    ...
    def generate(self) -> DINRailResult:
        ...
        # Main mounting plate
        base = Box(
            cfg.mount_width, cfg.mount_height, cfg.mount_thickness,
            align=(Align.CENTER, Align.CENTER, Align.CENTER)
        )
    ...
    def _add_rail_clips(self, base: Part) -> Part:
        ...
        result = base.fuse(top_positioned).fuse(bottom_positioned)
    ...
    def _create_fixed_clip(self) -> Part:
        ...
        result = Box(
            clip_width, 2, clip_height,
            align=(Align.CENTER, Align.CENTER, Align.MIN)
        )
        
        # Horizontal hook
        hook = Box(
            clip_width, lip_depth, 2,
            align=(Align.CENTER, Align.MIN, Align.MAX)
        ).moved(Location((0, -1, clip_height)))
        result = result.fuse(hook)
...
class WallMountGenerator:
    ...
    def _create_l_bracket(self) -> Part:
        ...
        result = Box(
            cfg.bracket_width, cfg.bracket_thickness, cfg.bracket_height,
            align=(Align.CENTER, Align.MIN, Align.CENTER)
        )
        
        # Shelf plate (horizontal, extending in +Y)
        shelf = Box(
            cfg.bracket_width, cfg.bracket_depth, cfg.bracket_thickness,
            align=(Align.CENTER, Align.MIN, Align.MAX)
        ).moved(Location((0, 0, -cfg.bracket_height / 2)))
        result = result.fuse(shelf)
    ...
    def _add_keyholes(self, bracket: Part) -> Part:
        ...
        if cfg.keyhole_count == 1:
            ...
            positions = [
                start_x + i * cfg.keyhole_spacing
                for i in range(cfg.keyhole_count)
            ]
        ...
        for x in positions:
            ...
            hole = Cylinder(
                cfg.keyhole_large_dia / 2, cfg.bracket_thickness * 2,
                align=(Align.CENTER, Align.CENTER, Align.CENTER)
            ).moved(Location((x, cfg.bracket_thickness / 2, 0))).rotate(Axis.X, 90)
            result = result.cut(hole)
            
            # Cut slot extending downward
            slot = Box(
                cfg.keyhole_small_dia, cfg.bracket_thickness * 2, cfg.keyhole_slot_length,
                align=(Align.CENTER, Align.CENTER, Align.MAX)
            ).moved(Location((x, cfg.bracket_thickness / 2, 0))).rotate(Axis.X, 90)
    ...
    def _add_ribs(self, bracket: Part) -> Part:
        ...
        for x_pos in positions:
            ...
            rib = Box(
                cfg.rib_thickness, rib_depth * 0.7, rib_height * 0.7,
                align=(Align.CENTER, Align.MIN, Align.MIN)
            ).moved(Location((
                x_pos,
                cfg.bracket_thickness,
                -cfg.bracket_height / 2 + cfg.bracket_thickness
            )))
...
class PCBStandoffGenerator:
    ...
    def generate(self) -> PCBStandoffResult:
        ...
        if cfg.hex_outer:
            standoff = self._create_hex_standoff()
        else:
            standoff = self._create_round_standoff()
    ...
    def _create_round_standoff(self) -> Part:
        ...
        # Base
        result = Cylinder(
            cfg.base_diameter / 2, cfg.base_height,
            align=(Align.CENTER, Align.CENTER, Align.MIN)
        )
        
        # Main column on top of base
        column = Cylinder(
            cfg.outer_diameter / 2, cfg.height,
            align=(Align.CENTER, Align.CENTER, Align.MIN)
        ...
        hole = Cylinder(
            cfg.inner_diameter / 2, total_height,
            align=(Align.CENTER, Align.CENTER, Align.MIN)
        )
        result = result.cut(hole)
